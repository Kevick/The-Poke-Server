...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnumIn("configKeys", ConfigManager::ALLOW_LEVEL_SYSTEM)
	...
	registerMethod("Game", "getLevelSys", LuaScriptInterface::luaGameLevelSys);
...
int LuaScriptInterface::luaGameLevelSys(lua_State* L)
{
	//Game.getLevelSys()
	if (!g_config.getBoolean(ConfigManager::ALLOW_LEVEL_SYSTEM) || g_config.getBoolean(ConfigManager::ALLOW_LEVEL_SYSTEM) == true) {
		return 0;
}
	return 1;
}
...
int LuaScriptInterface::luaMonsterTypeGetCatchRate(lua_State* L)
{
	//monsterType:getCatchRate()
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
